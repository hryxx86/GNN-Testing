...
**选择器**（`run_storya_cpre_select.py`，在已提交源码 044dd09 上运行）：168 候选（10 hc + 158 Alpha158，均 T−1）在调参 train 段 231 个特征日（2021-07-01→2022-05-31，标签终点 ≤ 2022-06-30）上算单特征日度 Spearman IC；τ=0.50 最小覆盖（`hc_mom12m` 85/231 未打分，其余 167 个满覆盖）；按 Plan-AAA 61 组取"成员 |时间均值 IC| 的均值"排名；top-15 组成员并集 → **48 列（5 hc + 43 Alpha158）**，与 C 的 51 列重叠 22 列、与 C5 重叠 4 列（source: `artifacts/storya_cpre_select/selection.json`）。τ=0.75 排名相同；τ=0 会把 `hc_mom12m` 排到第一（挤掉 RESI60）——冻结规则不变；这张表只度量"是否接纳那一个短历史特征"，不是选择器的一般稳定性（source: `artifacts/storya_cpre_select/selector_robustness.csv`）。组分数彼此非常接近：第 15 名 RESI60 0.03198 与第 16 名 IMIN10 0.03150 只差 0.00048，而单特征时间均值 IC 的标准误约 0.0099（日度 IC 中位 SD 0.1498 / √231），按 21 日标签重叠放大后约 0.045——60 个可排名组的分数全部落在第 15 名 ± 一个放大 SE 之内，top-15 切分是许多近似并列集合中的一个抽样（source: `artifacts/storya_cpre_select/group_scores.csv`, `feature_scores.csv`）。top-15 这个基数是从 Plan-AAA → C 的构造沿用的设计常量（Plan-AAA 的 top-15 本身是 test-informed 排名定的），为保持宽度可比而保留、未重新优化——排名本身是 pre-evaluation 的，**“pre-evaluation” 的适用范围**：只有选择器的**输入**以 2022-06-30 为界；**规则本身不是**——边际 |IC| 的组分数、复用的 61 组划分、top-15 的基数、合格阈值都沿用自 `analyze_plan_aaa_t1_diagnostic.py`（它自己的打分窗是面板最后 313 个标签日，落在评估期内）与 test-informed 的 Plan-AAA → C 构造；保留是为了宽度与口径可比、未重新优化，没有任何结论依赖于这个宽度（closeout EXPL-LEAK-03 / EXPL-STAT-07）。另一项披露：两个候选来源在构建期都做过 NaN→0 填充（`build_alpha158_features.py:362` 在保存 `_raw.npy` 之前；`run_step3_plan_z_part_a.py:113`），所以合格条件里的“有限值”判断是空的，缺失观测以填充零参与打分（被选中的 43 个 Alpha158 列构建期 NaN 率中位 1.7%、最高 3.3%）；实际起作用的是“非常数”判断——它正是排除 `hc_mom12m` 的原因（2022-01-28 前横截面全零）（closeout EXPL-LEAK-01）。

**运行**：Mac M4（mps，单次调用，git 46ca6e3，source_clean True），240/240 cell 完成且收敛，cell_id [3600, 3839]，每个 cell 都是冻结日历全长（749 天），frozen-HP 门通过（md5 a8fdfb8f）（source: `artifacts/storya_v21_family1_cpre/cpre_run_integrity.json`）。调参（30 trials，top-5 × 3 seeds，train ≤ 2022-06-30 / val 2022H2）：L0 冠军 val-IC +0.0311（决赛 +0.0302…+0.0311；LightGBM 在冻结参数下是确定性的，三个调参 seed 的 val-IC 完全相同，"三 seed 平均"对 L0 不含初始化信息），L1 三 seed 平均 +0.0135（决赛 +0.0014…+0.0135；L1 冠军单 seed 为 +0.034 / +0.012 / −0.006）；1 层 × 128，31,361 参数 vs C 的 31,745（source: `artifacts/storya_v21_family1_cpre/cpre_tuned_hparams.csv`, `experiments/storya_v21_tune/CPRE_L1.json`）。
...
Fold 结构（source: `experiments/storya_v21_main12_cpre/results.csv`, `artifacts/storya_v21_family1_cpre/cpre_ex_fold.csv`）：C-pre 的对比对 2025Q2（fold 9）敏感——该季 LightGBM IC 0.2042、MLP 0.0656，ΔIC −0.139（12 折中贡献最负；C、C5 在同一季是最大正贡献）。剔除该季后 C-pre ΔIC = +0.0099 [−0.0069, +0.0265]（p 0.198 / 0.348）；全期结果仍是主结果。C-pre 的 per-seed ΔIC 范围 −0.0314…+0.0173，5/10 与合并符号一致、1 次留一 seed 翻号（同一评估数据上的初始化敏感性）（source: `artifacts/storya_v21_family1_cpre/cpre_seed_robustness.csv`）。本条共发布 22 个 p 值：16 个是本次运行新算的 nominal p（无 BH），6 个是转载——C、B 的 L1−L0 两档 p 来自 confirmatory family1 表（auto-lag p 是 20 检验 BH 族的成员），C5 的两档来自 2026-09-11 的敏感性运行；它们是相关的规格/对比，不是独立复制；最小的是 C5 的 0.008，不是 C-pre（source: `artifacts/storya_v21_family1_cpre/cpre_tests_reported.json`）。
...
- All five finalists in each arm had positive three-seed average tuning ICs（与 C5 的决赛全负只是描述性差异）；these selected validation metrics establish neither independent validation nor search adequacy. 参数量相近（31,361 vs 31,745；source: `artifacts/storya_v21_family1_cpre/cpre_tuned_hparams.csv`）不等于有效容量、正则或优化相同。
- MDE 是近似的名义 80% 功效尺度，不是显著性阈值；比较表里 C 的 auto-lag 拒绝 vs 其块自助 CI 含 0、C5 的 CI 排零 vs lag-21 不拒绝，这些不一致照旧保留；C5 的排零本身是边界情形（|ΔIC| 0.0134 < 1.96×SE 0.0138，百分位不对称），不是稳健拒绝。
- 区间只反映日度方差：它是 10 个 seed 平均后的日度序列在已拟合模型条件下的不确定性，不重抽样特征选择、调参或训练 seed；seed 维度的披露是 per-seed 范围 −0.0314…+0.0173（跨度 0.049，比区间宽度 0.043 还大）。剔除 fold 9 的序列是把剩余观测跨过被删季度拼接的，HAC 窗与 21 日块跨过一个人为接缝——它只是诊断，全期结果为主。
...
**对论文的含义**：L1 的 "pending leak-free re-selection" 现在有了针对 MLP−LightGBM 的正面回答：在只用评估前信息选出的基底上，该对比的点估计为 −0.002、区间 [−0.026, +0.018]，C/C5/B 上的正向点估计没有重现，但配对区间全含 0——它不提供对 C 正向对比的独立确认，也不识别泄漏膨胀的大小。论文侧（H博士）需要：main.tex:290 / :998 / :1012 改正 "five groups survive strict T−1 re-ranking"（那是两种排名方法的交集）；承认已完成的 L0/L1 重选敏感性并保留对 C 原结果的限定；明确图/边对比的重选未做。B（无泄漏基底上 +0.0143，欠功效）仍是唯一的无泄漏锚点。（本段数字同上表；source: `artifacts/storya_v21_family1_cpre/cpre_comparison.csv`, `cpre_paired_contrast.csv`）
...
**调参冠军（选模指标，不是结果；source: `artifacts/storya_v21_family1_c5/c5_tuned_hparams.csv`、`experiments/storya_v21_tune/C5_{L0,L1}.json` top_table）**：L0 LightGBM {num_leaves 63, lr 0.0129, min_data_in_leaf 100, λ1 4.7e-4, λ2 0.016}；L1 MLP {lr 0.0092, wd 6.6e-5, dropout 0.3, hidden 32, 1 layer}，#params 2,337 @20 inputs（C 的 L1 冠军 31,745 @51）。披露：2022H2 调参窗上两臂**全部 5 个决赛配置**的 val-IC 均为负（L1 −0.0448…−0.0453，per-tune-seed −0.006/−0.045/−0.084；L0 −0.0121…−0.0122；C 对应冠军为 +0.074 / +0.060）——冻结超参符合协议但**不是经验证的最优**，选择未携带正信号；C5 的 MLP 比 C 的小约 14×。因此本条不把对比（或其与 C 的相似）归因于特征限制或容量。
...
| universe | ΔIC L1−L0（seed-averaged daily） | 21d block-boot 95% CI（对 10-seed 平均） | HLN p（NW auto lag） | HLN p（lag 21 = horizon） | MDE（≈2.8×SE） | per-seed 同号 | LOSO 翻转 |
|---|---|---|---|---|---|---|---|
...
读法（TP3 R-A-02 + closeout EXPL-STAT-01/02/03/05/06/10）：(i) 表中 HLN p 的 NW auto-lag（T=749 时 ≈6）是**实现默认值，不是协议 §6 指定项**（§6 只冻结 pair 族、BH q、block=21、MDE 规则）；21 日重叠标签下 ΔIC 在 lag 6 的自相关仍约 0.32，故并报 horizon 匹配的 lag-21 —— **lag-21 下 C5 / C / B 都未达 nominal 0.05**。(ii) lag-21 HAC SE 与 21d block-bootstrap SE 一致（C5：0.0068 vs 0.0070），但 5% 判定在 C5 上边际相反：百分位 CI 排零而 lag-21 p = 0.054；1.96×SE = 0.0138 > ΔIC 0.0134，CI 排零是**边界情形**，不是稳健拒绝。(iii) 三者分开说：C5 只在 auto-lag / 百分位 CI 读法下 5% 拒绝；C 在 auto-lag 下 BH 显著但 block-boot CI 含 0；B 是 non-detection；三者点估计都低于 ≈2.8×SE 的 80% 功效阈值。(iv) C5 的 nominal p 最小但点估计最小——排序由方差驱动（SE 0.0070 vs C 0.0079 / B 0.0098），不是更大的效应。(v) k/10 是同一数据上的 seed/初始化稳定性检查（非独立复制），k = 10 时 m = 0 是必然（同号均值去一不会翻转）。(vi) CI 针对 10-seed 平均序列（只含日间方差）；per-seed ΔIC 范围 +0.0029…+0.0250（source: `c5_seed_robustness.csv`）；0 塌缩 cell（`family1_stability.csv`）。C5 的 per-arm IC 水平（L0 0.020、L1 0.034）条件于 test-informed 选择，本文不把它们当作样本外表现引用。本条共发布 16 个 nominal p（source: `artifacts/storya_v21_family1_c5/c5_tests_reported.json`），未做多重校正。

**季度集中（TP3 R-A-01 + closeout EXPL-STAT-08；source: `artifacts/storya_v21_family1_c5/c5_ex_fold.csv`）**：fold 9（2025Q2）贡献 pooled ΔIC 的 **53%（C5）/ 44%（C）/ 31%（B）**（share = 62 × fold ΔIC / (749 × pooled ΔIC)）；它是 C5 与 C 的最大单折，在 B 是第二大（B 最大为 fold 7，+0.057）。C5 fold-9 ΔIC = +0.086；**剔除 fold 9 后 C5 ΔIC = +0.0069 [−0.0032, +0.0179]，p = 0.13（lag 21：0.23）**；C 同法 +0.0090 [−0.0052, +0.0228] p 0.125；B +0.0108 [−0.0089, +0.0300] p 0.143。C5 继承了 C/B 的季度集中，**不是均匀持续**。
...
**本条采用的表述（TP3 许可清单）**：在由两个 test-informed 重要性排名之交集选出的 20 列子集上，MLP−LightGBM 点估计为 +0.0134（95% 21d-block CI [+0.0008, +0.0283]，边界排零；nominal HLN p 在实现默认的 NW auto-lag 下 0.008、在 lag = horizon 下 0.054——后者与 C（0.063）、B（0.181）一样未达 0.05；10/10 seed 同号，故 LOSO 0 翻转为必然），对比 C +0.0148、B +0.0143；配对变化 +0.0013 [−0.016, +0.019]，点估计基本未变但区间不能区分"未变"与"减半/加倍"；三者的效应都低于近似 MDE；约一半来自 2025Q2；两臂调参 val-IC 为负；**本 sensitivity 不解决、不界定、也不估计 C 的 feature-selection leakage；由于选择用了 NN permutation 重要性，C/C5 内涉及 NN 臂的对比并非 selection-neutral；B 仍是 leak-free 特征基锚点**；只涉及 L0/L1，不提供 GNN 何时有帮助的新直接证据。**本条不采用、论文也不应采用的表述（TP3 清单）**："leak-free re-selection"、"survives T−1 re-ranking"、"definitive check"、"confirms/validates C"、"the advantage persists"（不带 fold-9 与 MDE 限定）、"robust to leakage"、"did not materially change / unchanged / equivalent"、单独引用 p = 0.008、任何容量归因、把 C5 per-arm IC 当样本外表现、"L1−L0 对比不受选择影响"。

**后续（待 H博士）**：(1) 论文 L1/附录两句改口径（改完再插 C5 段）；(2) **C5h 不必要**（TP3：对论断无影响）；(3) C-pre（pre-test 选择器，`docs/c5_rerun_brief_2026-09-10.md` §9.10 + Codex B-01 coverage 规则）**只在论文想对泄漏做量化陈述时才需要**，否则保持"suggestive；B 为锚；泄漏未量化"即可；(4) 可选 L2 层（T4 已就绪）。
...
### 5. Cost-口径 (gross/net) crosswalk — DESCRIPTIVE economic-sensitivity layer (net@10bps)

> **POSITIONING.** The §1–§3 headline is stated in IC (a gross rank metric). This subsection re-expresses each pre-registered claim under a net-of-cost economic口径. **Net Sharpe is a DESCRIPTIVE economic-sensitivity layer; IC remains the SOLE confirmatory metric** — no third BH-FDR confirmatory family is opened on Sharpe (would over-claim + add multiplicity; the project treats decile-L/S Sharpe as economic sensitivity, not robust alpha). Net口径 headline = 10bps (L1-one-way cost convention: net_ret = gross_ret − turnover_L1 × bps/10000, stored at run-time). Gross ΔIC + BH copied verbatim from `family1_dm_hln.csv`; **old E1/E6/pilot Sharpe numbers are NOT mixed into this confirmatory narrative**. Analyzer `compute_cost_confirmatory.py`; per-pair net ΔSharpe = fold-level seed-averaged (n≈12 fold blocks), stationary block bootstrap n_boot=5000; 33 degenerate C/L5s cells EXCLUDED (matches Family-1 `family1_stability.csv` 25 full + 8 partial). Reviewers: Touchpoint 2 `artifacts/reviews/2026-06-21_codex_code_cost_A.md` (PASS-WITH-CONCERNS — 1 CRIT rejected with code+data evidence + 2 hardening fixed); Touchpoint 3 `artifacts/reviews/2026-06-21_codex_results_cost_A.md` (PASS-WITH-CONCERNS — A-07 fixed, A-02 wording accepted).

The headline IC claims under a net economic口径 (source `artifacts/storya_v21_cost/cost_headline_crosswalk.csv` unless noted):
- **C-MLP > tuned LightGBM (L1−L0) HOLDS at net@10bps**: net ΔSharpe = +1.17, CI [+0.36, +2.08] excludes 0, LOFO sign-stable (source `cost_pairwise_dsharpe.csv` row C/L1-L0/10bps). Tuned LightGBM's own net Sharpe@10bps is **negative** (−0.22) while MLP's is strongly positive (+0.95) (source `cost_ladder_by_arm.csv` rows C/L0 & C/L1, cost_bps=10), so the economic separation is starker than the +0.0148 IC gap — **though the ΔSharpe itself erodes mildly with cost** (1.24 → 1.17 → 1.02 over 0 → 10 → 30bps; source `cost_pairwise_dsharpe.csv`) as MLP's higher turnover (2.90 vs 2.25; source `cost_ladder_by_arm.csv`) is paid down. C/MLP's per-arm net Sharpe is **heavy-tailed** (mean +0.95 vs median +0.18, max cell |Sharpe_gross|=31.7; source `cost_ladder_by_arm.csv`) → the MLP>LGB ranking rests on the **fold-level paired ΔSharpe (CI excludes 0)**, NOT the per-arm mean magnitude.
- **"graph doesn't help" (C L2−L1, corr-GAT < MLP) HOLDS at net**: net ΔSharpe@10bps = −0.72, CI [−1.42, −0.10] excludes 0 (source `cost_pairwise_dsharpe.csv`).
- **⚠️ COST-SENSITIVE — "news edge hurts" (C L3−L2) does NOT reproduce at net**: gross IC = −0.0123 (BH-reject, news harms *ranking*) but net ΔSharpe@10bps = +0.08 with CI [−0.77, +0.89] **straddling 0** and LOFO sign-UNstable (source `cost_pairwise_dsharpe.csv`); the 12 per-fold net ΔSharpe are **6 positive / 6 negative**, range −2.79 to +2.59 (source `cost_pairwise_folddeltas.csv` rows C/L3-L2/10bps). Reading — **PRIMARY signal = the net bootstrap CI**: the IC-口径 harm does not carry to the net economic口径, but the net difference is itself **indistinguishable from zero**. This is **NOT** evidence that news edges help economically; it is the only BH-significant claim whose口径 does not carry, and it is flagged cost-sensitive (consistent with the low-IC/high-Sharpe decoupling mechanism, 2026-06-15-a §2).
- **"recover above corr-GAT" arms hold at net**: C L5−L3 (+sector) net +0.88, CI [+0.12, +1.65] excludes 0; C L4/L5/L6 − L2 net all positive (source `cost_headline_crosswalk.csv`) — consistent with the §1 caveat against generalizing "graphs hurt".
- **Univ B**: the two BH-significant B contrasts (L2−L1 net −0.14; L3−L2 net −0.35) have net ΔSharpe CIs straddling 0 (source `cost_pairwise_dsharpe.csv`) — same direction as IC but economically underpowered, no net claim either way.
- **FC arm (Family-2) net**: all 6 contrasts' net ΔSharpe CIs straddle 0 (source `cost_fc_dsharpe.csv`) — consistent with Family-2's 0/6 BH-reject + 6/6 underpowered; no edge effect survives costs economically either.

**Net takeaway**: the two load-bearing predictive claims (neural NN beats trees; the graph adds nothing) survive the net economic口径; the one IC claim that flips口径 (news harms ranking) is a near-zero, fold-fragile net difference, honestly flagged and never promoted to "news helps". The §3 one-paragraph narrative is unchanged — cost口径 only sharpens it.

→ progress: 2026-06-21-a, 2026-06-21-b | plan: 2026-06-17 (FC 两族) + 2026-06-21 (cost口径) | analysis: 2026-06-21-a
...
**Item 7 (new, from Plan AAA T-1 diagnostic — H博士 verdict A; REVISED 2026-05-27-g per Codex T1 A-02 CRITICAL)**: Plan AAA T-1 stability diagnostic (`analyze_plan_aaa_t1_diagnostic.py`, output `artifacts/plan_aaa_t1_diagnostic/`) computed single-feature mean per-day spearman IC for 158 Alpha158 features under both leaky (raw) and T-1-shifted alignment on 313 test days; group-level proxy importance = mean(|feature_IC|) over members. **Artifact verdict (verbatim from `summary.md` line 27)**: **"LOW STABILITY (Plan AAA orig top-15 ∩ proxy-T1 top-15 = 5/15)"**. **Artifact action (verbatim from `summary.md` line 28)**: **"Universe C composition basis is leak-driven; full Plan AAA re-run required before submission, OR Universe C must be re-defined."** Three overlap metrics: (a) Plan AAA orig ∩ proxy-raw = 5/15 (proxy ≠ Plan AAA permutation framework by construction), (b) Plan AAA orig ∩ proxy-T1 = 5/15 (**KEY: LOW STABILITY under leak removal**), (c) proxy-raw ∩ proxy-T1 = 15/15 (direct leak-effect on proxy single-feature IC is small, ≤0.007 absolute). **H博士 2026-05-27 verdict A** decided to defer the full Plan AAA permutation re-run (~12-24h M4) to paper §Future Work. **§Limitations language MUST EXPLICITLY carry the LOW STABILITY verdict** (not "inconclusive" or "robust"): "Universe C composition basis is derived from a leak-affected Plan AAA permutation ranking; the T-1 diagnostic confirms LOW STABILITY (Plan AAA orig ∩ proxy-T1 = 5/15); a definitive re-ranking with T-1-shifted Alpha158 is deferred to future work and the current paper carries this caveat." The earlier "honest conclusion: the leak's direct effect on single-feature IC magnitude is small" framing remains TRUE but is SECONDARY to the LOW STABILITY verdict; the secondary framing must NOT lead the §Limitations paragraph. The E1 results themselves (using runtime T-1 shift in `build_universe_C`) remain leak-free as runtime artifacts; the §Limitations caveat targets the COMPOSITION BASIS (which 51 features were selected), not the E1 IC values.
