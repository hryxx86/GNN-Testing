...
**H博士 转来三项（2026-09-11）**：(1) 要不要跑 C-pre（教授要 "无泄漏重选"；B 锚 + C5 都回答不了）；(2) 图 `plan_aaa_t1_stability` 标题仍写 "survives / after strict T−1 leak correction"，要重画；(3) 本机 commit 未 push。

**已做**：
- (2) 图重画（progress 2026-09-11-f）：标题 → "Two importance measures disagree: 5/15 Plan-AAA top groups are also top-15 under the single-feature-IC proxy"；两个纯 hc 组标为 proxy unscored（不再算 "drops out"）；每点标不 shift 的 proxy 排名；脚本断言 proxy top-15 有无 shift 相同；`paper/iclr2027/figures/` 副本、gallery、figures/README 同步。**`main.tex` 未动**（:290 / :998 / :1012 仍旧口径，论文侧机器在改；caption 建议已随回复给出）。
- (1) C-pre 冻结方案 `docs/c_pre_plan_2026-09-11.md`（取代简报 §9.10）：选择窗 = TUNE_FOLD train 段 231 日（2021-07-01→2022-05-31，标签终点 ≤ 2022-06-30，val 2022H2 与测试期都在其后）；覆盖规则 τ=0.50（`hc_mom12m` 0.368 → UNSCORED；其余 167 个满覆盖）；组分数 = 成员 |时间均值 IC| 均值；复用 61 组；全 168 候选不与 C 求交；下游 = C5 流程，cell_id [3600, 4799]；三分支穷尽措辞、MDE 按自身 SE、不做 halved/doubled 推断；配对 C/B/C5。Codex TP1-A **PROCEED-WITH-FIXES（0 C / 2 M / 2 Cn，D1–D6 全 AGREE）**，修复已落入方案（progress 2026-09-11-g）。
- (3) push：C5 六个 commit + 本次 commit 推到 origin/main。

**建议：跑 C-pre。** 它是能正面回答 "无泄漏重选" 的最小实验；选择器信息边界 ≤ 2022-06-30，Codex 独立核对边界干净；成本 Mac ≈ 3 h（选择器 5 min + 调参 ≤ 40 min + 240 cell 2–3 h）或 T4 ≈ 1.5 h + TP2/TP3/closeout。**边界**（写进方案 §0）：只回答 L1−L0 的特征选择成分；不重评图/边对比；L8 面板构成限制不变；post-hoc、nominal p；结果不能把 C 的原结果变成干净的 confirmatory 证据。

**待 H博士**：
1. **C-pre go / no-go** + 确认 §8 D1–D6（默认已冻结；改任一项需重跑 TP1）。go 后顺序：TP2（含 Codex Round B）→ 提交选择器 → 选择 → 冻结 `UNIVERSE_CPRE_NAMES` → 调参 → 240 cell（Mac；给 T4 hostname 则 T4 主 + Mac 复现）→ 统计 → TP3 → 文档 → closeout。
2. 论文侧：main.tex:290 / :998 / :1012 改口径（`docs/c5_sensitivity_report_2026-09-11.md` §6.1）+ 新图 caption；C5 段落里 "does not distinguish an unchanged contrast from a halved or doubled one" 改为描述性或删去（Codex A-02）。
3. L2 层：仍建议不跑。
...
**待 H博士 决定（按紧迫度）**：
1. **论文改口径（必做，9/25 前）**：main.tex:290/:998/:1012 的 "only 5 of the 15 groups survive strict T−1 re-ranking" → "Plan-AAA top-15 ∩ single-feature-IC proxy top-15（proxy top-15 有无 T−1 shift 相同）"；撤回 "A re-run … is the definitive check, and we have not run it"；若插入 C5 段，按 analysis.md 2026-09-11-a 允许措辞（CI 先行、双 lag、< MDE、fold-9、负 val-IC、不解决泄漏、B 为锚；加一句 "selection used NN-based permutation importance, so within-C contrasts involving NN arms are not selection-neutral"）。
2. **C-pre（pre-test 选择器）**：只在论文想对泄漏做量化陈述时才需要（TP3 结论）；方案 `docs/c5_rerun_brief_2026-09-10.md` §9.10，需先冻结 Codex B-01 的 coverage 规则（`hc_mom12m` 252 日 warm-up）、组分数定义、分组复用 vs 重聚类；成本 ≈ 3 h + TP1/TP3。**C5h 不必要**。
3. **可选 L2 层**（C5 上 correlation-GAT 重调 5 h + 120 cell ~5 h，T4）：与论文当前主张关系弱，建议不跑。
4. **push**：本 session 4 个 commit（46b3b8c 补交 6 月改动、9008dbe C5 代码、14c284a 结果、+本轮修复/文档）未 push，待 H博士 确认。
...
**本轮完成**：`paper/main_jf_codex_compact.tex`（8pp 投稿候选）过 ultracode PaperJury；压缩判定 SAFE；scope-2（8 处压缩/实质修复）+ Tier-1（4 处结论加强，头条改为 leak-robust L2−L1<0）已落地并编译验证 8pp。见 progress/analysis 2026-06-30-a + `paper/.paper-review/LEDGER.json`。

**下一步（按优先级）**：
1. **投稿前必做（文字，不增页）**：填 author/affiliation/email + `\acmConference` 改双盲（class 选项 `nonacm`→投稿规范，handoff next_actions 既有项）。
2. **可选 reviewer-anticipation（8 个 open major，纯文字加 hedge，不重跑）**：I-01 软化 Table 1 "isolates one design choice" caption；I-02 在每处 MLP>GAT 解读补 30-trial 欠搜索 caveat；I-07/I-19 多重比较分层（补 joint-rate 说明或限定 claim 强度）；I-11 在 tab:dm 补关键 p 值；I-14 seed 稳健性一句；I-15 news-graph 构造无全样本拟合一句；I-17 把 survivorship caveat 扩到 level claims。
3. **Tier-3 需重跑（贵，可选 / future）**：leak-free Universe-C 重选（确认/退役 MLP>LightGBM）；trials-sensitivity sweep（解 I-02）；PIT 宇宙重建（解 I-17/L8）。
4. 决定 main.tex（9pp）与 compact（8pp）哪个为正式投稿源 → compact 为 8pp 候选，建议把 Tier-1 加强同步回 main.tex 或弃用 main.tex。

## 2026-06-24-a: Story A 论文 confirmatory 重构 — v2 草稿 Codex PASS，进入补全阶段 ⏳

**本轮完成**：v2 草稿（两族 + 6 confirmatory 图 + net-cost）全文重构、Codex T3 三轮 PASS（A→B→C）、2 张 exploratory 图（ListMLE 翻转 / Plan-AAA T-1）、nature-polishing 保守润色、旧 PILOT 草稿归档。详见 progress 2026-06-24-a。
...
| 2026-06-24 | 论文走 confirmatory 新文件 `docs/storya_paper_draft_v2.md`，旧 PILOT 草稿归档 | 旧草稿整篇 PILOT（5-fold / 单族 / α1-α4 / SPA 0.147），与 D-RERUN-12F confirmatory（12-fold / 两族 / L0-L7 / SPA 0.277-0.077）不可调和；H博士「全文重构·新文件」。探索性内容（loss-horserace / Plan-AAA T-1）**选择性保留**为 §5.7 exploratory（重建 2 图），余删。Codex T3 三轮 A→B→C PASS（`artifacts/reviews/2026-06-24_codex_results_{A,B,C}.md`）|
...
| 2026-06-26 | **M10 survivorship = DISCLOSE not rebuild (gap 14.8% names / 8.2% stock-days < 20% escalation line)** | Empirical Wikipedia audit (`analyze_m10_universe_gap.py`): 87 removed names absent from the fixed 501 snapshot = 14.8% names / 8.2% survivorship stock-days / 8.1% look-ahead / 16.3% two-sided composition mismatch. Below the pre-set 20% hard-escalation threshold → keep the fixed universe, disclose as a conditional (non-PIT) estimand in Methods §3.1 + Limitation L8; PIT rebuild → future work/QUEUE. Disclosure wording follows Codex TP3 specs (drop "cancels" → conditional-estimand; non-uniform attrition incl. SIVB/SBNY/FRC bank failures; absent-from-universe ≠ NaN-excluded). Rule9 TP2+TP3 both PROCEED. → progress 2026-06-26-a; analysis 2026-06-26-a. |
...
| 2026-09-11 | **C5 定性 = post-hoc、TEST-INFORMED feature-subset sensitivity（非 leak-free、非 confirmatory）；论文 "5 survive T−1" 与 "definitive check" 两句判为误表述，改口径待 H博士** | Codex TP1 A/B：选择器（proxy 最后 313 有效日 2024-09→2025-12；Plan AAA 5 折测试季 2024-04→2025-06）都在 12 折测试期内；proxy top-15 有无 T−1 shift 相同（Claude 亲自核实）。Codex option 1 照跑 + 改定性；option 2 C-pre 作提案 |
| 2026-09-11 | **T4 = C5 主结果（结果前预先声明），Mac = 设备复现** | H博士 指示 "gpu完全没在用"；两份 pooled ΔIC 0.0134 vs 0.0132 一致（L0 逐位相同，L1 cell corr 0.951） |
| 2026-09-11 | **C5 结果措辞按 TP3（fallback）许可/禁止清单：CI 先行 + 双 HAC lag + < MDE + fold-9 集中 + 负 val-IC 披露 + 不解决泄漏；C5h 不必要；C-pre 仅在要量化泄漏时** | TP3 0 C + 4 M + 3 Cn 全部亲自核实（ex-fold-9 +0.0069 p 0.13；paired MDE 0.025 > C 效应；决赛配置 val-IC 全负；Plan AAA 用 NN permutation 重要性） |
| 2026-09-11 | **Codex 额度中断（"try again at 4:40 AM"）→ TP2-B、TP3 走 finance-gnn-reviewer fallback（如实记录）** | Rule 9 Fallback 条款；第一次 fallback 又撞 Claude 月度 429，重置后完成 |
